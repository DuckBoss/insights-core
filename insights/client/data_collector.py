...
class DataCollector(object):
    ...
    def _write_rhsm_facts(self, post_proc):
        logger.info('Writing RHSM facts to %s...', constants.rhsm_facts_file)

        hn_block = []
        for k, v in post_proc.hn_db.items():
            hn_block.append({'original': k, 'obfuscated': v})

        kw_block = []
        for k, v in post_proc.kw_db.items():
            kw_block.append({'original': k, 'obfuscated': v})

        ip_block = []
        for k, v in post_proc.ip_db.items():
            ip_block.append(
                {
                    'original': post_proc.int2ip(v),
                    'obfuscated': post_proc.int2ip(k)
                })

        facts = {
            'insights_client.hostname': post_proc.hashed_fqdn,
    ...
    def run_collection(self, conf, post_proc, branch_info, blacklist_report):
        ...
        '''
        def _get_spec_obfs(symbolic_name, post_proc):
            if symbolic_name:
                symbolic_comp = get_component_by_name('insights.specs.Specs.{0}'.format(symbolic_name))
                spec_obfs = symbolic_comp.obfuscate if symbolic_comp else []
                return set(spec_obfs) & set(post_proc.obfuscate)
        ...
        rm_commands = post_proc.redact.get('commands', [])
        rm_files = post_proc.redact.get('files', [])

        for c in conf['commands']:
            symbolic_name = c.get('symbolic_name')
            command = c.get('command')
            spec_obfs = _get_spec_obfs(symbolic_name, post_proc)
            if command in rm_commands or symbolic_name in rm_commands:
                ...
                for s in cmd_specs:
                    ...
                    self.archive.add_to_archive(cmd_spec, spec_obfs, post_proc)
            ...
            if f['file'] in rm_files or symbolic_name in rm_files:
                ...
                for s in file_specs:
                    ...
                    if s['file'] in rm_files:
                        ...
                        self.archive.add_to_archive(file_spec, spec_obfs, post_proc)
                ...
                if symbolic_name in rm_files:
                    ...
                    for s in glob_specs:
                        if s['file'] in rm_files:
                            ...
                            self.archive.add_to_archive(glob_spec, spec_obfs, post_proc)
                            collection_stats[s['file']] = {
    ...
    def done(self, conf, post_proc):
        """
        Do finalization stuff
        """
        if self.config.obfuscate and post_proc:
            # generate RHSM facts
            self._write_rhsm_facts(post_proc)
